...
export async function executeCode(code: string, testCases: TestCase[], language: string): Promise<TestResult[]> {
  console.log(`🚀 Starting code execution for ${language} with ${testCases.length} test cases`)
    ...
    switch (language) {
      ...
      default:
        ...
        return testCases.map((testCase) => ({
          ...
          actual: "",
          passed: false,
          error: `${language.charAt(0).toUpperCase() + language.slice(1)} execution is not supported yet.`,
          executionTime: 0,
        }))
    }
  } catch (error) {
...
async function executeJavaScriptCode(code: string, testCases: TestCase[]): Promise<TestResult[]> {
  ...
  for (const testCase of testCases) {
    ...
    try {
      ...
      if (!functionMatch) {
        throw new Error("No function found in code. Make sure to define a function.")
      }
      ...
      console.log("📊 Comparison:", { actual: actualStr, expected: expectedStr, passed })
      ...
      if (error instanceof Error) {
        ...
        if (errorMessage.includes("undefined")) {
          ...
          errorMessage = "Variable or function not defined. Check your variable names and function calls."
        } else if (errorMessage.includes("Cannot read property")) {
          errorMessage = "Trying to access property of null/undefined. Check your object references."
        }
      }
...
function parseTestInput(input: string): { params: string[]; paramValues: string[] } {
